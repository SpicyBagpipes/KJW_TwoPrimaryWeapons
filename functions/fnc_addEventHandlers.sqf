...
player addEventHandler ["Killed", {
	...
	_unit setVariable [QGVAR(secondPrimaryInfo),[]];
	_unit setVariable [QGVAR(primaryPrimaryInfo),[]];
	_unit setVariable [QGVAR(secondPrimaryEquipped),false];
	private _objects = _unit getVariable [QGVAR(currentWeaponObjects),[]];
	if (_objects isEqualTo []) exitWith {}; // No other guns.
	{
		deleteVehicle _x;
	} forEach _objects;
	_unit setVariable [QGVAR(currentWeaponObjects),[]];
